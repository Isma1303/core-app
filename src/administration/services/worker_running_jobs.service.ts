...
import { IWorkerRunningJob } from '../interfaces/worker-running-job.interface'

export class WorkerRunningJobsService implements DataService<IWorkerRunningJob> {
    private readonly URL_BASE: string = '/worker-running-job'

    public async getRecord(key: string | number): Promise<IWorkerRunningJob> {
        try {
    ...
    public async getRecords(offset: number, limit: number, orden?: string): Promise<IWorkerRunningJob[]> {
        try {
    ...
    public async createRecord(data: IWorkerRunningJob | never[]): Promise<IWorkerRunningJob | never[]> {
        try {
    ...
    public async updateRecord(key: string | number, data: IWorkerRunningJob | Record<string, any>): Promise<IWorkerRunningJob[]> {
        try {
    ...
    public async search(conditions: Condition[], offset: number, limit: number, orden?: string): Promise<IWorkerRunningJob[]> {
