...
export class UsersService implements DataService<User> {
    private readonly URL_BASE: string = '/user'

    public async getRecord(key: string | number): Promise<User> {
        try {
    ...
    public async getRecords(offset: number, limit: number, orden?: string): Promise<User[]> {
        try {
    ...
    public async createRecord(data: User | never[]): Promise<User | never[]> {
        try {
    ...
    public async updateRecord(key: string | number, data: User | Record<string, any>): Promise<User[]> {
        try {
    ...
    public async search(conditions: Condition[], offset: number, limit: number, orden?: string): Promise<User[]> {
    ...
    public async validateActualPassword(actualPassword: string): Promise<boolean> {
        try {
            const { data: apiResponse } = await serverAPI.get(`${this.URL_BASE}/validateActualPassword/${actualPassword}`)
