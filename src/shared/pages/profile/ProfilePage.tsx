...
export const ProfilePage = (): JSX.Element => {
    ...
    const onUserFormSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (window.confirm('¿Está seguro de que desea actualizar la información de usuario?')) {
            ...
            userService.updateRecord(user_id!, data).then((response) => {
                if (response) {
                    toast.success('La información de usuario ha sido actualizada correctamente')
                }
            })
        }
    }
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-8">
            <Card className="flex flex-col items-center justify-center p-8 border-border/50 shadow-sm transition-all hover:shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <form onSubmit={onUserFormSubmit} className="space-y-6">
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    <label className="text-sm font-medium text-foreground/90">Usuario</label>
                                    <Input
                                        className="h-10 border-border/50 bg-muted/50 focus-visible:ring-1 focus-visible:ring-foreground/20 transition-all"
                                        type="text"
                                        value={user.user || ''}
                                        disabled
                                    />
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-sm font-medium text-foreground/90">Correo Electrónico</label>
                                    <Input
                            ...
                            <Button
                                type="submit"
                                className="w-full h-10  active:scale-[0.98] transition-all"
                            >
                                Guardar Cambios
                            </Button>
                        </form>
                ...
                <Card className="border-border/50 shadow-sm">
                    <CardHeader>
                        ...
                        <CardDescription>Actualiza y protege tu contraseña.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={onPasswordFormSubmit} className="space-y-6">
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    <label className="text-sm font-medium text-foreground/90">Contraseña Actual</label>
                                    <Input
                                        className="h-10 border-border/50 focus-visible:ring-1 focus-visible:ring-foreground/20 transition-all"
                                        type={passwordType}
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-sm font-medium text-foreground/90">Nueva Contraseña</label>
                                    <Input
                                        className="h-10 border-border/50 focus-visible:ring-1 focus-visible:ring-foreground/20 transition-all"
                                        type={passwordType}
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-sm font-medium text-foreground/90">Confirmar Contraseña</label>
                                    <Input
                                        className="h-10 border-border/50 focus-visible:ring-1 focus-visible:ring-foreground/20 transition-all"
                                        type={passwordType}
                                ...
                                <div className="flex items-center space-x-2 pt-2">
                                    <Checkbox
                                        ...
                                        onCheckedChange={(checked) => {
                                            setShowPasswordValue(checked === true)
                                            setPasswordType(checked === true ? 'text' : 'password')
                                        }}
                                    />
                                    <label
                            ...
                            <Button
                                type="submit"
                                variant="outline"
                                className="w-full h-10 border-border/50 hover:bg-foreground/5 transition-all"
                            >
