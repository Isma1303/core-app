...
import { useScreenService } from "@/shared/services/screen.service";
import {
    Alert,
    AlertDescription,
    AlertTitle,
} from "@/components/ui/alert"
import { useCalendar } from "@/ddg/hooks/useCalendar";
...
export const CalendarComponent = () => {
  const { loadUserEvents, confirmAttendance, notifyAbsence } = useCalendar();
  ...
  const calendarView =
    isMobile && selectedView === Views.WEEK ? Views.AGENDA : selectedView;


  useEffect(() => {
    loadUserEvents().then((events) => {
  ...
  const filteredEvents = useMemo(() => {
    return allEvents.map((event: any) => {
      ...
      return {
        ...
        start: dayjs(
          `${eventDate.format("YYYY-MM-DD")} ${event.start_time}`,
        ).toDate(),
        end: dayjs(
          `${eventDate.format("YYYY-MM-DD")} ${event.end_time}`,
        ).toDate(),
      };
    });
  }, [allEvents]);
  ...
  return (
     <>
      ...
      <div
        className="surface-card shadow-1 border-round-xl overflow-hidden"
      ...
      >
          ...
          <div className="flex align-items-center w-full md:w-auto">
            ...
            >
              {viewOptions.map((option) => (
                ...
                //         <span className="font-medium text-900">Option 1</span>
                //         <span className="text-600">Subtitle</span>
                //     </div>
                // </SelectItem>
                <div></div>
              ))}
        ...
        <div style={{ height: isMobile ? "60vh" : "70vh", padding: "10px" }}>
          ...
          <Calendar
            ...
            views={
              isMobile
                ? [Views.DAY, Views.AGENDA, Views.MONTH]
